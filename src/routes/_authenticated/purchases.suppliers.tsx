...
export const Route = createFileRoute("/_authenticated/purchases/suppliers")({ component: SuppliersList });
...
function SuppliersList() {
  ...
  const filtered = suppliers.filter(s => !ql || s.name.toLowerCase().includes(ql) || (s.mobile || "").includes(ql));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-baseline justify-between flex-wrap gap-2">
        <h1 className="text-3xl md:text-4xl font-black">{t("suppliers")}</h1>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button size="lg" className="h-12 font-bold"><Plus className="w-5 h-5 mr-1" /> {t("addSupplier")}</Button></DialogTrigger>
          <DialogContent>
